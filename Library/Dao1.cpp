...
map<int, char*> Dao::transCharPtr2Map(string tableName, int id, char * row)
{
	...
	for (int i = 1; i < colums; i++) {
		...
		//猥琐的办法解决,第一列不能查询的问题
		res.insert(pair<int, char*>(i - 1, t));
		//正常方法
...
bool Dao::insert_into(const string tableName, vector<char*> &v)
{
	...
	//猥琐的办法解决第一列不能查询问题，
	int step = t_info[0], i = 1;
	//设定第0列填充 0， 防止 update 函数检查属性出错
	char *fill0Column = new char[t_info[0]];
	memset(fill0Column, 0, t_info[0]);
	memcpy(res, fill0Column, t_info[0]);
	...
	try {
		...
		for (map<int, char *>::iterator it = obj.begin(); it != obj.end(); ++it) {
			...
			if (it->first == -1 || it->first == 0)
				continue;
			//处理0列问题
			if (memcmp(it->second, v[(it->first )- 1], column_len[i++]) != 0) {
			//正常写法
...
vector<map<int, char*>> Dao::select(const string tableName, vector<pair<int, char*>>& v)
{
	...
	try {
		...
		if (v.size() == 0) {
			...
			//猥琐写法
			vector<int> t1 = _table.find((v[0].first )+ 1, v[0].second);
			for (int i = 1; i < v.size(); i++) {
				Utils::getIntersection(t1, _table.find((v[i].first) + 1, v[i].second));
			}
